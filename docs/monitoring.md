...
| Feature              | **Event Monitoring**                                                                                                                | **Business Transaction Monitoring (BTM)**                                                                                                                      |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Purpose**          | To emit **technical events** during message flow execution for auditing, troubleshooting, logging, and visibility.                  | To track an **end-to-end business transaction** across multiple flows, systems, or stages.                                                                     |
| **Focus**            | What *happened* inside a specific message flow node (e.g., node entered, exited).                                                   | What *business step* a particular transaction completed (e.g., “Order validated”, “Payment authorised”).                                                       |
| **Level**            | Low-level **technical** monitoring.                                                                                                 | High-level **business** process tracking.                                                                                                                      |
| **What it captures** | Node-level events such as: <br>• NodeInput <br>• NodeDetail <br>• NodeException <br>• Custom events                                 | Business events such as: <br>• Transaction start <br>• Transaction progress <br>• Transaction success/failure <br>• Timing and SLA compliance                  |
| **Granularity**      | Very detailed technical information.                                                                                                | Aggregated business-level information.                                                                                                                         |
| **Usage scenario**   | • Debugging <br>• Identifying where a message failed <br>• Auditing node behaviour <br>• Sending events to Splunk/ELK/ACE Dashboard | • Following a customer order across systems <br>• SLA monitoring of business flows <br>• Tracking a payment journey <br>• Fraud detection/business correlation |
| **Output**           | Monitoring events (JSON/XML) emitted per node based on Monitoring Profile.                                                          | A complete “Transaction Timeline” with checkpoints and correlated data.                                                                                        |
| **Correlation**      | Does not track a multi-step end-to-end business transaction (only individual events).                                               | Groups multiple events under a single **Transaction ID** across flows and systems.                                                                             |
| **Configuration**    | Defined in the **Monitoring tab** of a message flow (Monitoring Events).                                                            | Configured via **BTM Events**, Monitoring Profiles + extra business correlation setup.                                                                         |
| **Target audience**  | Developers, operations teams.                                                                                                       | Business analysts, support teams needing business-level visibility.                                                                                            |
